...
If both j0 and v1-v2 are 0, the machine interrupts and passes the value of v1=v2 as an opcode to the IO engine. The engine splits the opcode into 3 equal parts, starting from the highest-value trit: flags, additional parameter and the operation code. If the tape width is not divisible by 3, the remaining 1 or 2 lowest trits are discarded. If the opcode is 0, the machine halts unconditionally. 

The IP engine evaluates its position on the tape (-3 or +3 from the position before the interrupt) as a jump address and two other cells next to it on both sides of the tape as two operands, similar to the main machine. The order of the operands depends in the sign of the opcode:
...
(9 16086946250976080 17943922394188172 14852728792888700 0 0 0 0 0 0 -1 1 36 3 3 3 -17 Start->0 -17 -19 -6 -3 -12 0 -12 -23 -12 -9 -18 0 -18 -30 -18 -15 -24 0 -24 -37 -24 -21)
