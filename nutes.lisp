...
(defun create-tape
    (length &optional (position 0) (counter 0))
		   (make-tape
		    :position position
		    :initial 0
		    :vector (make-array length :element-type '(signed-byte 64))
		    :length length
		    :halted nil
		    :special nil
		    :counter counter))
...
(defun one-step (tape)
  ...
  (let*
      ((p (tape-position tape))
       ...
       (sign (sign-sum a+ a-)))
    (declare (type (signed-byte 64) p))
    (declare (type (signed-byte 64) l))
    (declare (type (signed-byte 64) p+))
    (declare (type (signed-byte 64) p-))
    (declare (type (signed-byte 64) j))
    (declare (type (signed-byte 64) j-))
    (declare (type (signed-byte 64) j+))
    (declare (type (signed-byte 64) j0))
    (declare (type (signed-byte 64) a+))
    (declare (type (signed-byte 64) a-))
    (declare (type (signed-byte 64) sub))
    (declare (type (signed-byte 8) sign))
